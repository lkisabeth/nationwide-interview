...
class BedrockPromptManagementClient:
    ...
    def execute_prompt(
        ...
        """Execute a prompt with the Bedrock Runtime API.
        
        Args:
            prompt_id: ID of the prompt to execute
            variable_values: Optional map of variable values
            version_number: Optional version number to execute
        ...
        try:
            ...
            if variable_values:
                kwargs["variableValues"] = variable_values
                
            if version_number:
                kwargs["promptVersion"] = version_number
                
            response = self.bedrock_runtime_client.invoke_model_with_prompt_management(**kwargs)
            
            # Decode the response body
            model_response = response.get("modelResponse", {})
            if "body" in model_response:
                import json
                ...
                if hasattr(body, "read"):
                    ...
                    model_response["body"] = json.loads(body_content)
